...
## Parche 7 — 2026-04-21 (apoyos: estructura final webhook + reparto por bloques/solapes)

### Objetivo del parche
Implementar la estructura final de respuesta del webhook de apoyos y aplicar distribución de propina por bloques de 5 minutos considerando solapes de personas en rango de tiempo.

### Archivos implicados
- `js/apoyos.js`  
  **Tipo:** lógica aislada (core de reparto).  
  **Cambios:**
  - consume respuesta en formato:
    ```json
    [{"data":[{"empresa_id":"...","apoyo_responsable_id":"...","total_propina_periodo":0}]}]
    ```
  - construye segmentos de propina por diferencias acumuladas,
  - distribuye por bloques de 5 minutos entre presentes por bloque,
  - redondea a múltiplos de 50 COP,
  - ajusta residual al responsable para cuadrar total,
  - mantiene propina de apoyos ineditable (`readonly`).

- `js/cierre_turno.js`  
  **Tipo:** datos fuente para reparto.  
  **Cambios:** mantiene inhabilitación manual de propina y envía 3 campos adicionales de rango:
  - `rango_hora_inicio_simple`,
  - `rango_hora_fin_simple`,
  - `rango_fecha_hora_inicio`.

- `docs/2026-04-21_superadmin_flujo_global_usuarios_empresas_y_7_parches.md`  
  **Tipo:** documentación operativa.

### Notas para casos de apoyos en periodos distintos
- El cálculo ya considera solapes por bloque temporal de 5 minutos.
- Si dos apoyos comparten franja, esa franja se reparte entre responsable + apoyos presentes en ese bloque.
- Si un apoyo no comparte bloques con otro, su participación queda separada por su presencia real en el eje temporal.

### Emergencia / reversión
1. Volver temporalmente al reparto simple (igualitario) reemplazando `distributeByTimeline` por lógica previa.
2. Si hay inconsistencias en respuesta webhook, usar fallback de `total_propina_periodo=0` por apoyo y mantener propina del responsable intacta.

### Check (logs de estado)
- ✅ Webhook de apoyos consumido en estructura final definida por negocio.
- ✅ Reparto por bloques de 5 minutos aplicado con solapes.
- ✅ Propina de apoyos sigue bloqueada para edición manual.
- ✅ Se conservan 3 campos extra en payload de apoyos.
