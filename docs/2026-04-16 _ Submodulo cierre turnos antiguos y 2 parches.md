...
## Parche 2 (2026-04-16): rediseño visual y responsive del cierre de turnos antiguos

### Objetivo del parche
Alinear la interfaz del submódulo con la identidad visual del cierre de turno normal, mejorar distribución en escritorio y móviles, y eliminar riesgo de navegación horizontal accidental.

### Archivos afectados
- `cierre_turno/turnos_anteriores.html`
  - Reestructuración semántica de cabecera, bloques y sección de totales.
- `css/cierre_turnos_anteriores.css`
  - Rediseño completo: tarjetas de totales, tabla financiera por filas, media queries para móvil y ajustes de espaciado/jerarquía visual.
- `js/cierre_turnos_anteriores.js`
  - Ajuste del render de totales y estructura de filas financieras para el nuevo layout responsive.

### Reversión de emergencia del parche
1. Restaurar versión anterior de estos tres archivos desde el commit previo a este parche.
2. Verificar que los selectores `data-field` y `data-col` sigan presentes para no romper validaciones y payloads.
3. Validar en móvil que no exista scroll horizontal.

### Estado (log)
- [x] Vista escritorio: mejor jerarquía visual y lectura.
- [x] Vista móvil: distribución vertical de campos y sin desborde horizontal forzado por grillas rígidas.
- [x] Lógica funcional de consulta/distribución/envío: se mantiene compatible.
