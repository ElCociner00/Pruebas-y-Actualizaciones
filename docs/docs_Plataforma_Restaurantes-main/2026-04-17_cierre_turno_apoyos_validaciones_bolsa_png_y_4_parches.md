...
- `2026-04-17_cierre_turno_apoyos_validaciones_bolsa_png_y_4_parches.md`
...
## Parche posterior #3 (2026-04-19) — Cambio de tiempo por rango en apoyos

### Objetivo puntual del parche #3
1. Reemplazar el selector de duración fija por selección de rango (inicio/fin con AM/PM, minutos en intervalos de 5 y fecha).
2. Mantener compatibilidad con payload previo calculando `tiempo_minutos` y `tiempo_texto` desde el rango.
3. Enriquecer payload con campos adicionales de rango (`rango_hora_inicio`, `rango_hora_fin`, `rango_hora_unificado`, `fecha_rango`).

### Cambios aplicados
- `js/cierre_turno.js`
  - se removió el selector fijo de 16 horas y se agregó lectura de rango inicio/fin por fila de apoyo.
  - nueva función de cálculo para transformar rango en duración (minutos y texto).
  - validación de apoyos ahora exige rango completo válido.
  - `buildApoyoPayload` conserva `tiempo_minutos`/`tiempo_texto` y añade campos de rango.
- `css/cierre_turno.css`
  - estilos para render de doble cuadro (inicio/fin) y selects internos del rango.

### Reversión rápida del parche #3
- En `js/cierre_turno.js`, restaurar selector único `data-field="tiempo"` y quitar helpers de rango.
- En `css/cierre_turno.css`, quitar clases `.apoyo-rango-*`.
- En payload, eliminar campos `rango_hora_*` y `fecha_rango` si se vuelve a esquema antiguo.


---

## Parche posterior #4 (2026-04-20) — Restauración de acceso superadmin sin tenant

### Objetivo puntual del parche #4
1. Recuperar acceso funcional al módulo superadmin cuando el usuario no tiene `empresa_id`/`tenant_id`.
2. Reconectar resolución de contexto usando `system_users` por correo/id autenticado.

### Cambios aplicados
- `js/session.js`
  - en fallback de `getContextFromTables`, si el usuario no existe en `usuarios_sistema`, ahora consulta `system_users` (y fallback `system_user`) por `id` y `correo`.
  - cuando existe coincidencia, construye contexto `admin_root` sin empresa (`empresa_id: null`) y `super_admin: true`, permitiendo acceso a módulos globales de gestión.

### Reversión rápida del parche #4
- En `js/session.js`, eliminar la rama de fallback que consulta `system_users`/`system_user` y volver al retorno `null` cuando no exista `usuarios_sistema`.
